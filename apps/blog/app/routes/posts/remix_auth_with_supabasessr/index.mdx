...
description: "Remixと@supabase/ssrでGoogle認証を行う"
category: "Supabase"
tags:
  - Cloudflare
  - Remix
...
## SupabaseとRemixでGoogle認証をする
### Supabase Clientと認証処理の作成
...
「Sign In」ボタンを押下すると実行される`Action`では`signInWithGoogle`でログインし、コールバック先へリダイレクトしています。  
```ts
...
export const action = async ({ request, context }: ActionFunctionArgs) => {
  ...
  // また、headersは付与忘れるとコールバック処理がうまくいかないので注意
...
### コールバック処理の作成
次にコールバック先のページを作成します。  
ここでは`auth/callback.ts`を作成し、以下のとおり記載します。  
...
<ExLinkCard url="https://supabase.com/docs/guides/auth/social-login/auth-google?queryGroups=framework&framework=remix&queryGroups=environment&environment=server&queryGroups=platform&platform=web#application-code" />  
...
また、発展形として、下記スクラップでRemixからHonoにJWTを投げてユーザ情報を取得する等行っているので、もしよければご参照ください。  
<ExLinkCard url="https://zenn.dev/sorinaji/scraps/53d2ab1a57e3ae" />  
