...
全然関係ないですが、このブログはMDXで記事を書いているのですが、コードブロックにハイライトを付けるとCloudflare Pagesだけでビルドが失敗する現象が発生してます。。  
ローカルでViteする分には問題ないのですが、、なぜ？  
お見苦しくて申し訳ないですが、コードはGitHubを参照くださいm(_ _)m