---
icon: "🛠️"
permalink: "this_blog_architecture"
...
古来からの言い伝えで雑なアウトプット先としてWordpressブログを使ってましたが、以下の課題がありました。
...
元々、NestJs+Prisma等を使って構築しようとしていましたが、Next.jsでSSするだけで十分かつコストを抑えられるので、この構成を採用しました。  
...
Contentfulのコンテンツ取得には、[GraphQL Content API](https://www.contentful.com/developers/docs/references/graphql/)と、Apollo Clientを使用してCodegenで下記の様に、型定義とQueryが自動生成させてます。
...
Contentfulにも、Wordpressの様な投稿画面はありますが、個人的に使い辛かったので、記事はMarkdownで書いてGitで管理したいと考えました。
...
他にやりたいこともあるので、ゆっくり余生を過ごすような思いで直していきます〜！
