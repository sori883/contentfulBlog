...
import { getH } from 'lib/canvas/configOgText';
...
export const generateOgImage = async (title: string): Promise<Buffer> => {
  // font を登録
  const font = path.resolve(current, 'src/lib/canvas/NotoSansJP-Bold.otf');
  ...
  // 元になる画像を読み込む
  const src = path.resolve(current, 'src/lib/canvas/og_image.png');
